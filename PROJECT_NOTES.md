...
- Screenshot alat (`computer` sa `action:"screenshot"`) je nepouzdan u ovoj sesiji ("Browser pane is not displayed"). `javascript_tool` (DOM/computed style/getBoundingClientRect/getAttribute) je glavni način verifikacije. Za vizuelnu proveru PRE ubacivanja u kod (npr. tekst wrap, boje, gradient kontrast) — koristi lokalni render preko `sharp`/SVG (Node skripta → PNG → `Read` alat), pa tek onda ubaci u komponentu.
- Korisnik komunicira na srpskom, sadržaj sajta je na engleskom.
