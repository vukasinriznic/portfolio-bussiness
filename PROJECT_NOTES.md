...
## Sesija (2026-08-17) — Kontakt forma, load/scroll animacije svuda, Lenis nav-scroll, SEO (sitemap/canonical/Afera Digital)

### `/kontakt` stranica + prava funkcionalna forma (Resend email)
- Nova ruta `src/app/kontakt/page.tsx` + `src/components/ContactForm.tsx` (client) + `src/app/api/contact/route.ts` (POST handler).
- Polja: ime i prezime, email, budžet (bedževi koji se biraju, NE dropdown — `PillGroup` sub-komponenta u `ContactForm.tsx`), hitnost (isto bedževi), kratak opis. Budžet/hitnost su `useState` kontrolisani (NE native `<select>`), pa validacija mora biti ručna (`showValidation` state) jer HTML5 `required` ne radi na custom dugmićima.
- Slanje ide preko **Resend** (`resend` npm paket, INSTALIRAN trajno ovaj put — ostaje u `package.json`, za razliku od `qrcode`/`sharp` koji su bili privremeni). API key u `.env.local` kao `RESEND_API_KEY` — **ne sme se commit-ovati** (već pokriveno sa `.env*` u `.gitignore`, provereno `git check-ignore`). ⚠️ **Ja (Claude) NE SMEM da unosim API ključeve/tokene ni kad korisnik to eksplicitno traži** — pravilo bez izuzetka, korisnik mora sam da nalepi u fajl. Ovo se ponovilo 2x ovu sesiju (korisnik je insistirao "ubaci slobodno", odgovor ostaje isti).
- Odredište: `vukasin.afera@gmail.com` (hardkodovano u `route.ts` kao `TO_EMAIL`), `from: "Portfolio kontakt <onboarding@resend.dev>"` (Resend sandbox domen), `replyTo` = email posetioca. **Resend sandbox ograničenje**: dok se ne verifikuje sopstveni domen, sme se slati SAMO na email kojim je nalog registrovan — zato je korisnik namerno registrovao Resend nalog baš sa `vukasin.afera@gmail.com`.
- **Testirano kraj-do-kraja i POTVRĐENO da radi** (korisnik je primio test mejl) — vidi ispod u "napomene za oprez" o testing gotchas koje su usporile ovo testiranje.
- Dizajn forme prošao kroz VIŠE rundi izmena u istoj sesiji: veličina teksta (20px→16px), border-radius (rounded-3xl→rounded-md, "skoro oštre ivice"), boja pozadine forme (belo→`#262626` kao About sekcija→NAZAD na belo sa `border border-[#525252]` 1px), pozadina cele stranice (belo→`#262626`→NAZAD na `#F6F6F6`/`bg-surface` sa `/cta-waves.svg` talasima kao na CTA sekciji), social red na dnu prebačen da koristi `var(--name-width)` "veliki kontejner" umesto uskog `max-w-3xl` forme. **Trenutno finalno stanje**: svetla pozadina sa talasima, bela forma sa tankim border-om, input polja ISTE boje kao forma ali sa `border-border` da se vizuelno razdvoje.
- Avatar u social redu (bedž "Vukašin Riznić") je imao bag: `object-cover` bez `object-position` seče vrh glave na kružnom cropu (izvor slike `profile.png` je 1024×1536 portret, centriran crop pokazuje sredinu tela ne lice). Probano `object-top` fix — **korisnik je tražio da se VRATI kako je bilo** ("vrati sliku kao sto je bila pre nego sto smo je dirali"), pa je `object-cover` bez `object-top` OSTAO finalno stanje na `CTA.tsx` i `kontakt/page.tsx`. Ne diraj ovo opet bez pitanja.
- Sva 4 primarna dugmeta ("Započnimo saradnju" ×3, "Pišite mi") sada vode na `/kontakt` umesto `#contact`/`mailto:`.

### Load + scroll-reveal animacioni sistem — NOVA arhitektura, koristi se SVUDA
Korisnik je tražio da se load/scroll animacije modeluju po `https://www.dymasalfin.web.id/`. **Tehnika koja je upalila**: umesto nagađanja, inspektovan je live DOM tog sajta preko `javascript_tool` — Framer Motion ostavlja inline `style="opacity:...; transform:...; transition:..."` na elementima, pa se TAČNE brojke mogu pročitati direktno (`document.querySelectorAll('[style*="translateY"]')` itd.). **Ovo je opšta tehnika vredna ponavljanja** kad god treba da se "izmeri" tuđa animacija umesto nagađanja.
- Nađene tačne vrednosti sa referentnog sajta: `opacity:0→1`, `transform:translateY(28px)→none`, `transition: 0.7s cubic-bezier(0.22, 1, 0.36, 1)`, stagger ~80ms između elemenata u nizu.
- Novi `src/lib/motion.ts` — deljene konstante `fadeUp` (y:28, duration:0.7, ease:[0.22,1,0.36,1]), `staggerContainer` (staggerChildren:0.08, delayChildren:0.05), `viewportOnce` ({once:true, amount:0.2}).
- Novi `src/components/motion/`: `Reveal.tsx` (pojedinačan fade-up na scroll, `whileInView`), `RevealGroup.tsx` + `RevealItem.tsx` (stagger grupa, roditelj+deca preko `variants`), `LoadGroup.tsx` (isto kao RevealGroup ali `initial/animate` — okida se NA MOUNT, ne na scroll, za elemente iznad fold-a poput header-a).
- ⚠️ **`Reveal`/`RevealGroup`/`RevealItem` su `"use client"` komponente ALI mogu da se importuju i renderuju iz SERVER komponenti** (Next.js composition pattern) — zato `/projekti/page.tsx` i `/kontakt/page.tsx` (koje imaju `export const metadata`, moraju ostati server komponente) i dalje mogu da koriste animacije bez konverzije cele stranice u `"use client"`.
- ⚠️ **`Reveal` NEMA `as` prop** — renderuje se uvek kao `motion.div`. Za `<h1>`/`<h2>` NE menjaj tag (gubi se semantika/SEO), nego wrap-uj: `<Reveal><h2>...</h2></Reveal>` (div oko heading-a, ne heading zamenjen div-om).
- Primenjeno na: **Hero** (load animacija usklađena na iste brojeve kao ostatak sajta — bila je `y:24, ease:[0.16,1,0.3,1]`, sad `y:28, ease:[0.22,1,0.36,1]`), **Header** (novi mount-triggered load animacija za bedž/nav/dugme, PRVI PUT dodato — pre nije imao nikakvu animaciju), **Work** (naslov→kartice stagger→dugme), **Services accordion** (VIDI POSEBNO ispod, drugačije od ostalih), **CTA** (bedž→naslov/paragraf→dugme→social red), **`/projekti`** (naslov+brojač→kartice stagger), **`/kontakt`** (naslov+paragraf→forma→social red), i top bar-ovi ("Nazad"+"Dostupan za saradnju" na `/projekti` i `/kontakt`, preko `LoadGroup`+`RevealItem`, ISTI mount-pattern kao Header). **About namerno NETAKNUT** — zadržava svoj poseban GSAP scroll-jack tretman, ne treba ga ujednačavati.
- **Services accordion ima DRUGAČIJI pattern od svega ostalog**: korisnik je eksplicitno tražio "kartica po kartica dok skrolujem" — svaki red ima SVOJ `whileInView` (nezavisan IntersectionObserver), NE deljen roditelj-stagger. Ovo je namerno drugačije od `RevealGroup`/`RevealItem` patterna (koji sve stagger-uje odjednom kad RODITELJ uđe u view) — testirano na uskom viewport-u (500px) da se potvrdi nezavisno okidanje po redu, ne svi odjednom.
- **Bag nađen i ispravljen**: `Header.tsx` je koristio `var(--name-width, 80rem)` fallback koji se na prvom render-u (pre nego što `NameWidthSync` izmeri pravu vrednost) razlikuje od stvarne izmerene vrednosti i do ~100px na širokim monitorima — izgledalo je kao "trzanje/bag" kad se sadržaj header-a pomeri udesno čim se JS izvrši. Fix: dodata CSS `transition-[background-color,max-width] duration-300 ease-out` na header, pretvara nagli skok u glatku tranziciju.

### Lenis smooth-scroll za klik na navbar linkove
Ranije: klik na `#work`/`#services`/`#about`/`#contact` je koristio browser default instant skok (Lenis se koristi SAMO za wheel/touch skrol, ne za anchor klikove po default-u).
- Novi `src/lib/lenis.ts` — čuva referencu na aktivnu Lenis instancu (`setLenis`/`getLenis`), postavlja se iz `SmoothScroll.tsx` na mount/unmount.
- `Header.tsx` `handleNavClick` presreće klik, `e.preventDefault()`, poziva `lenis.scrollTo(target, {offset, duration, easing})` — sa fallback-om na `scrollIntoView({behavior:"smooth"})` ako Lenis nije dostupan (npr. `prefers-reduced-motion`, SmoothScroll se tad uopšte ne mount-uje).
- `offset: -HEADER_HEIGHT - 16` (80px) da sekcija ne bude sakrivena ispod fiksnog mobile header-a.
- Korisnik je tražio "isto kao na dymasalfin.web.id" — INSPEKCIJA je pokazala da TAJ sajt koristi NATIVNI `scroll-behavior:smooth` (ne Lenis/custom lib), sa ručnim offset-om (~95px). **Odlučeno da OSTANE na Lenis-u** (ne prebacivati na native) jer bi native smooth-scroll bio nekonzistentan sa Lenis-ovom custom fizikom koja već vodi SVE ostalo skrolovanje na sajtu — isti krajnji UX rezultat (glatko skrolovanje + header clearance), bolji mehanizam za OVAJ sajt.
- Posle korisnikovog feedback-a "vise smooth, easing in & out": dodato `duration: 1.4` i custom `easing: (t) => t < 0.5 ? 4*t*t*t : 1 - Math.pow(-2*t+2, 3)/2` (easeInOutCubic) — NAMERNO drugačija kriva od `[0.22,1,0.36,1]` ease-OUT koji se koristi svuda drugde za load/scroll-reveal, jer ease-in-out daje taj "plovljiv" osećaj specifično za scroll-to-section, ne za reveal-on-enter.

### SEO dodaci: sitemap, robots, canonical, Afera Digital signali
- `src/app/sitemap.ts` i `src/app/robots.ts` — Next.js file-convention rute, generišu `/sitemap.xml` i `/robots.txt` automatski (`MetadataRoute.Sitemap`/`MetadataRoute.Robots` tipovi). Sadrže sve 3 rute (`/`, `/projekti`, `/kontakt`).
- Novi `src/lib/site.ts` — `SITE_URL` konstanta izvučena iz `layout.tsx` (bila lokalna `const siteUrl`), sad deljena između `layout.tsx`, `sitemap.ts`, `robots.ts`. **Domen potvrđen od korisnika: `https://www.vukasinriznic.me`** (kupiće ga uskoro, još nije live).
- `alternates: { canonical: "/" }` / `"/projekti"` / `"/kontakt"` dodato u sva 3 `metadata` exporta — sprečava da Vercel-ov `*.vercel.app` preview domen (koji uvek postoji uporedo sa pravim domenom) razvodni SEO signal kao "duplirani sadržaj".
- **Afera Digital je korisnikova SOPSTVENA firma** (ne tuđa, potvrđeno pitanjem) — korisnik želi da se njegov portfolio poveže sa pretragom "aferadigital". Firma NEMA sopstveni sajt, SAMO Instagram. Urađeno: `personJsonLd.worksFor` dodat, social link labela `lib/links.ts` promenjena sa "Afera"→"**Afera Digital**" (i `Hero.tsx` `mobileSocialPosition` mapa ključ ažuriran da se poklapa). ✅ **AŽURIRANO u sledećoj sesiji istog dana**: pravi Instagram handle je `@afera.digital` (`https://www.instagram.com/afera.digital/`) — `href` promenjen svuda (`lib/links.ts`, `layout.tsx` `sameAs` + `worksFor.sameAs`). Preporuka za username data korisniku pre nego što je napravio profil: izbegavaj `-` (Instagram ne dozvoljava), izbegavaj `.rs` TLD stil (izgleda kao pogrešno ukucan sajt), preferiraj tačku nad donjom crtom za čitljivost — korisnik je probao `aferadigital` (zauzeto) pa otišao na `afera.digital`.
- SEO impact iskreno objašnjen korisniku: sitemap/robots/canonical su "housekeeping" (brže/pouzdanije indeksiranje), NE ranking boost sami po sebi. Najveći sledeći lever bi bio pravi sadržaj (opisi projekata umesto placeholder teksta) i backlink-ovi, ne dalji meta tag tweaking.
- **Nije urađeno (spomenuto, nije implementirano)**: OG slika je i dalje kvadratna (1254×1254) umesto idealnog 1200×630 landscape formata za deljenje linkova na Facebook/LinkedIn/Twitter.

## Sesija (2026-08-17, nastavak) — Mobile animacioni bagovi (video analiza), FastAPI, Afera Digital Instagram, responsive fix-evi

### ⚠️⚠️ MOBILE ANIMACIJA JANK — dva stvarna bag-a nađena preko analize video snimka, PROČITAJ AKO DIRAŠ REVEAL ANIMACIJE
Korisnik je poslao `.mp4` snimak ekrana telefona gde je Hero dugme "sec kalo" (jank/flicker), kartice na Work/`/projekti` se NISU otkrivale jedna po jedna (izgledale su već potpuno vidljive kad uđu u ekran), i accordion redovi su takođe "sekli". **Tehnika koja je otkrila uzrok**: pošto `Read` alat ne može da čita video fajlove, PRIVREMENO je instaliran `ffmpeg-static` (`npm install --no-save ffmpeg-static`, isti "temp install → koristi → deinstaliraj" pattern kao ranije za `sharp`/`qrcode`), preko njega izvučeni frejmovi na 20-25fps za sumnjive delove snimka (`ffmpeg -ss <start> -t <trajanje> -vf "fps=N" ...`), pa vizuelno pregledani frejm-po-frejm preko `Read` alata. **Ovo je opšta tehnika vredna ponavljanja za bilo koji budući "izgleda da seče/buguje" izveštaj koji dolazi kao video** — screenshot alat ne hvata tranzicije, ali izvučeni frejmovi to rešavaju.

Nađena DVA odvojena, stvarna uzroka:

1. **CSS `transition-all` na ISTOM elementu koji nosi Framer Motion `variants`** — Services accordion red (`motion.div variants={fadeUp}` + `className="... transition-all ${ACCORDION_EASE_CSS} ..."`) i Hero mobile "Započnimo saradnju" dugme (`motion.a variants={item}` + `className="... transition-all hover:..."`) su OBA imala Framer-ov JS-driven opacity/transform I CSS `transition-all` na jednom te istom DOM elementu. Browser je pokušavao da animira ISTU promenu preko dva sistema odjednom → vidljivo treperenje/posivljivanje teksta (crni tekst prolazi kroz sivu fazu jer opacity nakratko "poskoči" umesto monotono da raste). **Fix: RAZDvoji u DVA elementa** — spoljašnji `motion.div` NOSI SAMO `variants={fadeUp}` (bez ijedne CSS transition/hover klase), a unutra plain `<div>`/`<a>` nosi SVE stilove/hover efekte/CSS transition nepromenjeno. Primenjeno na `Services.tsx` (red omotan u dodatni `motion.div`) i `Hero.tsx` (mobile dugme omotano u `motion.div`, `<a>` unutra ostaje plain). **PRAVILO ZA UBUDUĆE: NIKAD ne stavljaj `variants`/`animate` Framer Motion prop DIREKTNO na element koji ima i CSS `transition-*` klasu (pogotovo `transition-all`) — uvek ih razdvoji u wrapper + child.** `Reveal`/`RevealGroup`/`RevealItem`/`LoadGroup` komponente iz `src/components/motion/` ovo već rade ispravno by design (uvek renderuju ČIST wrapper div bez ikakvih drugih klasa) — bag se desio SAMO na dva mesta gde je Framer Motion ručno (bez tih helper komponenti) stavljen direktno na već-stilizovan element.
2. **`viewportOnce.margin` (dodat u prošloj sesiji da spreči teoretski "instant scroll skip" bag) je pravio DRUGI, gori problem**: `margin: "20% 0px 20% 0px"` je terao IntersectionObserver da okine animaciju dok je element JOŠ UVEK ~160px ISPOD vidljivog dela ekrana — na brzom mobilnom skrolu (realan flick gest), cela 0.7s animacija bi se ZAVRŠILA pre nego što element uopšte uđe u vidno polje, pa je izgledalo kao da kartice "samo iskoče" gotove, bez ikakvog fade-up efekta. **Fix: margin UKLONJEN, vraćeno na originalni `{ once: true, amount: 0.2 }`** (ista vrednost koja je RANIJE ovog dana testirana i potvrđena kao ispravna, PRE margin eksperimenta). Pošto SAV skrol na sajtu ide kroz Lenis (koji UVEK interpoluje frame-po-frame, nikad instant skok), teoretski "stuck forever" edge-case koji je margin trebalo da reši se REALNO ne dešava kroz normalno korišćenje sajta — bio je to nepotreban trade-off. **Ako se ikad ponovo razmišlja o margin/amount podešavanjima za `viewportOnce`, imaj na umu ovaj trade-off: veći margin = manje šanse za "zaglavljen" element, ALI i veća šansa da se animacija completely odigra OFF-SCREEN i bude nevidljiva korisniku na brzom skrolu.**

### Ostale izmene ove sesije
- **CTA naslov**: "Imate projekat na umu?" → "**Treba vam web sajt?**" (korisnik dao smer "nesto tipa treba vam web sajt", odobrio direktno bez dodatnih opcija).
- **Web Development opis** (`lib/data.ts`): dodat pomen FastAPI-ja — "Brzi, responzivni sajtovi i web aplikacije izrađeni pomoću modernih alata poput React-a, Next.js-a i FastAPI-a." (korisnik dao TAČAN tekst, ubačen bez izmena).
- **`/kontakt` mobile font veličine**: naslov 56px→**32px** na mobile (`sm:text-[56px]` ostaje za desktop), paragraf 20px→**16px** (`sm:text-[20px]`), sav sadržaj forme (labele, input polja, bedževi, dugme) 16px→**14px** (`sm:text-[16px]`) — sve preko standardnog Tailwind mobile-first pattern-a (base klasa = mobile, `sm:` prefiks = desktop).
- **`/projekti` naslov umanjen**: 40px/64px/80px (mobile/sm/lg) → **32px/56px** (mobile/sm, lg tier uklonjen) — sad se poklapa sa veličinom naslova koju već koriste Work/Services sekcije, radi doslednosti.
- **"Afera Digital" pilula na uskim ekranima**: ispod **372px** širine ekrana prikazuje se skraćeno "**Afera**" (bez "Digital") da se izbegne prelom teksta u dva reda unutar pilule (korisnik poslao screenshot bag-a). Implementirano preko Tailwind arbitrary breakpoint-a `hidden min-[372px]:inline` na `<span>` koji sadrži " Digital" deo teksta, SAMO u `SocialLinks.tsx` (deljena komponenta za CTA/`/kontakt`/desktop Hero social red). **Hero-ov MOBILE floating bedž sistem je NAMERNO netaknut** — on je odvojena, hardkodovana implementacija u `Hero.tsx` (ne koristi `SocialLinks.tsx`), pa korisnikov zahtev "svuda osim u hero" je automatski ispunjen bez dodatnog koda. Referenca: iPhone 15 viewport = 393px (iznad praga, prikazuje puno ime), iPhone SE = 375px (takođe iznad).
- **Instagram klik pokazuje "popup blocking" traku** — korisnik prijavio, ANALIZIRANO i POTVRĐENO da NIJE bag u kodu: `SocialLinks.tsx` koristi potpuno standardan `<a target="_blank" rel="noopener noreferrer">`, identičan LinkedIn/GitHub linkovima koji rade bez problema. Upozorenje dolazi od SPECIFIČNOG browsera/aplikacije korisnika (ne standardni Chrome/Safari), van naše kontrole. Korisnik odlučio da OSTAVI `target="_blank"` kako jeste (ponuđena alternativa — ukloniti `target="_blank"` — ODBIJENA).
- Sve iz ove sesije JE pushovano (commits `d16e572`, `0c4366f`, `039fcd6`) — `.env.local` sa `RESEND_API_KEY` OSTAJE lokalno, nikad se ne commit-uje (potvrđeno gitignored).

## Sesija (2026-08-20) — Pravi projekti, redizajn kartica, performance cleanup, font/hydration debugging

### Pravi projekti ubačeni (napokon, umesto placeholder-a)
- `src/lib/data.ts` `projects` niz sad ima **7 pravih projekata**, svaki sa `title`, `tags` (2 taga, na engleskom kao i naslovi usluga — namerno), `image` (path u `public/projects/`), `url` (link ka live sajtu), i opciono `featured: true`.
- **Podaci prikupljeni sa**: GitHub repo liste (`github.com/vukasinriznic?tab=repositories`) preko `WebFetch` za opise/tehnologije, i live sajtova (Matiola, Patriot Winery) direktno preko `WebFetch` pošto GitHub repo nije imao opis za njih. Live URL-ovi za svaki repo nađeni preko pojedinačne repo stranice ("Website" polje u About sidebar-u) — **NE VEROVATI SLEPO tom polju**: TahoService je imao MRTAV Railway link tamo (`tahoservice-production.up.railway.app` → 404), ispravan je bio u README-u (`tahoservice.onrender.com`, sad sa `/login` putanjom po korisnikovom finalnom linku). **Uvek proveri `curl -I` / HTTP status pre nego što ubaciš eksterni link u kod.**
- Konačna lista (redosled u `data.ts`): Ancora (`ancora-ai.vercel.app`, featured), Matiola (`matiola-flowershop.vercel.app`, featured), Patriot Winery (`patriot-winery.webflow.io`, featured), Elevation Worship (`elevationworshiphome.webflow.io`), TahoService (`tahoservice.onrender.com/login`), Provenance (`provenance-blockchain-vukasinriznic.webflow.io`, featured), TealFather (`tealfather.webflow.io`).
- **MealMuse je POTPUNO uklonjen** (bio je u ranijoj verziji liste) — korisnik eksplicitno tražio da se izbaci, i slika `mealmuse.png` obrisana iz `public/projects/` (ne samo unfeatured, stvarno izbrisan iz podataka i sa diska).
- `Work.tsx` (početna) prikazuje SAMO `projects.filter(p => p.featured)` (trenutno: Ancora, Matiola, Patriot Winery, Provenance — Elevation Worship je bio featured pa je ZAMENJEN Provenance-om po korisnikovom zahtevu). `/projekti` stranica prikazuje SVE (`projects` bez filtera), sa ispravnom srpskom pluralizacijom broja projekata.

### `ProjectCard.tsx` redizajniran — prave slike, klik vodi na live sajt
- Placeholder gradijent+inicijali UKLONJEN, zamenjen pravim `next/image` screenshot-om (`fill`, `object-cover object-top`).
- **Ceo card je sad `<a href={project.url} target="_blank" rel="noopener noreferrer">`** (ranije `<article>`, nije bio klikabilan) — `group` klasa premeštena sa `article` na `a` da hover efekti i dalje rade.
- **Odnos slike menjan VIŠE PUTA ovu sesiju** (istorija bitna ako se opet dira): prvo `aspect-[11/5]` (2.2:1, tačan odnos svih screenshot-ova, BEZ sečenja ali "prekratko" po korisniku) → korisnik eksplicitno tražio VEĆU visinu ali rekao "visinu ne diraj, samo širinu" kad sam pokušao da promenim mehanizam merenja visine na jednom drugom mestu (Services, vidi ispod) → **finalno: `aspect-video` (16:9 ≈ 1.78)**, blagi kompromis (umereno sečenje sa strane umesto ozbiljnog, dovoljno visoka kartica).
- **Oštrina slika — pravi uzrok bio je DVOSTRUK**: (1) `sizes` prop je pretpostavljao pogrešan broj kolona za grid u kom se kartica NALAZI (Work.tsx ima 2 kolone, ne 3 kao default) → sad `Work.tsx` prosleđuje `sizes="(min-width: 640px) 62vw, 100vw"`, `/projekti` koristi default `"(min-width: 1024px) 42vw, (min-width: 640px) 62vw, 100vw"` — **oba SVESNO uvećana ~1.25-1.3x iznad "sirovog" CSS width-a kartice** da kompenzuju `object-cover` efekat kad je odnos kontejnera (16:9) UŽI od odnosa slike (2.2:1) — u tom slučaju `object-cover` skalira sliku PREMA VISINI kontejnera, što zahteva ŠIRU sliku nego što `sizes` (koji računa samo CSS širinu) inače komunicira Next-u, pa bez ovog uvećanja Next šalje suviše malu (mutnu) verziju. **OVO JE OPŠTI PRINCIP vredan pamćenja: kad god je `object-cover` kontejner UŽI od prirodnog odnosa slike, `sizes` mora biti veći od kontejnerove sirove CSS širine, faktor ≈ (odnos_slike / odnos_kontejnera).** (2) Next.js 16 default `images.qualities` je SAMO `[75]` (breaking change), pa je `quality={90}` prop bio TIHO odbačen dok nisam dodao `images: { qualities: [75, 90] }` u `next.config.ts`.
- Sve ovo primenjeno i na Work.tsx-ovu upotrebu ProjectCard-a (featured kartice na početnoj) i na `/projekti`.

### Services accordion — preview slike umesto "Pregled" teksta
- `lib/data.ts` `services` niz sad ima `image` polje po usluzi, mapa u REDOSLEDU koji je korisnik dao: Web Design→provenance, Web Development→tahoservice, E-Commerce→matiola, Performance & SEO→elevationworship, Maintenance & Support→tealfather.
- `Services.tsx` preview kutija (desno od naslova kad je accordion item otvoren) sad renderuje `<Image src={service.image} .../>` umesto `<span>Pregled</span>`.
- **Korisnikovo pravilo, ZAPAMTI**: "možeš menjati ŠIRINU kutije, visinu NE DIRAJ" — visina MORA ostati `top-6 bottom-6 lg:top-[48px] lg:bottom-[48px]` (rasteže se da popuni visinu otvorene kartice, ZAVISNA od dužine opisa teksta). Širina je uvećana sa `w-40 lg:w-56` (160/224px) na `w-52 lg:w-72` (208/288px). **Pokušao sam prvo da promenim NA `aspect-[11/5]` + `top-1/2 -translate-y-1/2` (fiksni odnos, centrirano) — korisnik je to EKSPLICITNO odbio i tražio da se vrati.** Ne ponavljaj tu grešku — ako opet zatreba veća/manja slika, diraj SAMO `w-*` klase, nikad `top-*`/`bottom-*`/`aspect-*` na ovoj kutiji.
- Pošto visina VARIRA (zavisno od teksta), a širina je fiksna, kutija ima RAZLIČIT odnos širina/visina po usluzi → isti "sizes mora biti veći od sirove širine" princip kao gore, ali OVDE nema jednog tačnog faktora (menja se po stavci) — rešeno grubom silom: `sizes="1024px"` (fiksna velika vrednost, dovoljna za bilo koju realnu kombinaciju širine/visine na ovoj kutiji, Next će svakako kapirati na stvarnu rezoluciju izvorne slike).
- Kutija je bila PRETESNO uz X (zatvori) dugme (`right-[80px]` ostavljalo samo ~2px razmaka) — pomerena na `right-[105px]` (27px razmak, izmereno i potvrđeno). Naslov usluge (`<span>`) max-width uvećan da prati (`md:max-w-[calc(100%-325px)] lg:max-w-[calc(100%-425px)]`) da se ne preklapa sa slikom.
- Testirano da je i dalje `hidden md:block` (nema slika na mobile-u, po zahtevu).

### Performance/memory cleanup
- Obrisano: `public/next.svg`, `globe.svg`, `window.svg`, `vercel.svg`, `file.svg` (default `create-next-app` boilerplate, NIKAD korišćeni), `public/images/Logo_black.png`, `Logo_white.png`, `logo.svg` (ostaci iz perioda kad je pravljen favicon, `Logo.tsx` komponenta koristi INLINE SVG path, ne ove fajlove — favicon fajlovi `icon.png`/`apple-icon.png`/`favicon.ico` u `src/app/` su već generisani, ovi source fajlovi više nisu potrebni).
- `clsx` npm paket uklonjen (`npm uninstall clsx`) — proverio `grep` da se nigde ne koristi u `src/`.
- `Cursor.tsx` mousemove/mouseleave listener-i dobili `{ passive: true }`.
- ⚠️ **VAŽNO OTKRIĆE**: `public/projects/` (prave screenshot slike projekata) **NIKAD nije bio pushovan na GitHub pre ove sesije** — sve prethodne izmene vezane za projekte su bile samo lokalne, live Vercel sajt do sada UOPŠTE nije imao prave slike/podatke. Sad je konačno commit-ovano i pushovano.
- Razmatrao sam i odbacio: kompresiju `vukasin_hero.png` (1.16MB, koristi se i kao OG/Twitter slika) — zaključio da OG slika NE utiče na brzinu sajta za posetioce (samo je preuzimaju social-media crawleri kad generišu preview linka, ne redovni korisnici), pa to NIJE realan "performance" problem vredan rizika diranja profilne fotografije. Ako se ikad želi manji OG payload, napraviti POSEBAN, manji fajl za OG tagove, ne dirati `vukasin_hero.png` koji se koristi i na stranici (kroz `next/image`, već optimizovan na serve-u).
- Razmatrao i odbacio: prepisivanje `Header.tsx` scroll+`getBoundingClientRect()` dark-mode detekcije u `IntersectionObserver` — teorijski bolje, ali dovoljno komplikovano (treba dinamički `rootMargin` baziran na viewport visini) da rizik od suptilnog buga nadmašuje skromnu dobit (već ima `{passive:true}`, cleanup na unmount, i browseri inače usklade scroll evente sa frame rate-om). Ako se ikad ozbiljno napada "performance", ovo je sledeće mesto za pogled.

### ⚠️⚠️ Font-loading bug na localhost-u (NE bug u kodu) — dijagnostička tehnika vredna pamćenja
Korisnik je prijavio da Hero "izgleda skupljeno" na localhost-u dok je live Vercel sajt normalan, i pomislio da je uzrok njegova sopstvena izmena `title` stringa (em-dash→pipe u `layout.tsx`) — TO JE BILA SLUČAJNOST U VREMENU, ne uzrok.

**Pravi uzrok**: Next.js DEV server (ne browser, ne build) preuzima Google Fonts (Syne, Inter) fajlove SA GOOGLE-OVOG SERVERA u trenutku prvog zahteva i sam ih hostuje (`next/font/google` u dev modu). Ako u tom trenutku ima problema sa konekcijom, dev server TIHO padne na fallback font ("Syne Fallback"/"Inter Fallback" — Next-ov auto-generisani metrički-usklađeni zamenski font) i **ne pokušava ponovo sam od sebe** dok se proces ne restartuje. Fallback font ima DRUGAČIJE (uže) dimenzije karaktera od pravog Syne-a, pa `NameWidthSync.tsx` (koji MERI stvarnu širinu renderovanog imena da postavi `--name-width`) izmeri ZNAČAJNO manju vrednost (760px ili čak 316px umesto očekivanih ~1288px na 1280px viewport-u) → svi kontejneri vezani za tu CSS promenljivu (Hero, Services, CTA, `/projekti`, `/kontakt`) se "skupe".

**Dijagnostička tehnika koja je otkrila uzrok** (ponovi ako se opet pojavi sličan "sve je manje/čudno" bug bez očigledne veze sa kodom):
1. Izmeri `getComputedStyle(document.documentElement).getPropertyValue('--name-width')` — ako je drastično drugačije od očekivanog (~1280-1400px na desktop viewport-ima), sumnjaj na font/merenje problem, ne CSS/layout kod.
2. Proveri `[...document.fonts].map(f => ({family: f.family, status: f.status}))` — ako se pojavljuje SAMO "X Fallback" a ne pravo ime fonta, font se nije učitao.
3. Proveri mrežne zahteve za `.woff2` fajlove — ako ih NEMA UOPŠTE (ne 404, nego potpuno odsutni), browser ni ne pokušava da ih preuzme, znači CSS `@font-face` nikad nije dobio pravi `src` (server-side problem, ne browser).
4. **Najpouzdaniji dokaz**: pogledaj `.next/dev/logs/next-development.log` (`tail -N`) — tražiti `"Failed to download X from Google Fonts"` i `"There was an issue establishing a connection while requesting https://fonts.googleapis.com/..."` WARN zapise. Ovo DIREKTNO potvrđuje server-side fetch failure, eliminiše nagađanje.
5. ⚠️ **Konekcija testirana IZ MOG bash alata je USPEŠNO prošla (`curl` 200 OK, 3/3 puta) dok je Next dev server I DALJE prijavljivao failure u logu** — moja mreža i mreža Node dev procesa NISU nužno isti put (različit proxy/env/DNS keš). **Ne veruj da je "internet OK" samo zato što JA mogu da pingujem Google iz Bash-a — proveri STVARNO stanje browser fontova/loga, ne pretpostavljaj da se problem rešio.** Ovo je greška koju sam napravio usred sesije (rekao korisniku "trebalo bi da je fixed" na osnovu SVOG curl testa, a font i dalje nije radio) — **UVEK proveri `document.fonts` u SVEŽOJ kartici pre nego što tvrdiš da je font problem rešen, ne veruj sopstvenom mrežnom testu iz Bash-a kao dokazu.**
6. **NIJE bug u browseru niti u kodu** — `git restore` na poslednji commit NIJE pomogao (probano, i dalje slomljeno) jer problem NIJE u kodu uopšte, nego u dev-server procesu. Rešenje je isključivo restart dev servera (i po potrebi `rm -rf .next` za potpuno čist pokušaj preuzimanja) — **korisnikov browser screenshot koji izgleda "normalno" MOŽE biti keširan/stara stranica, ne pouzdan dokaz da je live problem rešen; uvek zatraži hard refresh (Ctrl+Shift+R) ili sam proveri u SVEŽOJ kartici.**
7. **Produkcija (Vercel) NIKAD nije bila pogođena** — `next build` peče fontove trajno u fajlove pri build-u, ne zavisi od žive konekcije ka Google-u u runtime-u. Ovo je ISKLJUČIVO dev-mode problem.

### Hydration warning fix — `suppressHydrationWarning` na `<body>`
Odvojen, nepovezan problem od gornjeg (obe stvari su se desile otprilike istovremeno, korisnika zbunilo). Next.js dev overlay je prijavio "A tree hydrated but some attributes... didn't match" sa `cz-shortcut-listen="true"` atributom na `<body>`. **Ovo je potpis ColorZilla browser ekstenzije** (ubaci taj atribut na `<body>` PRE nego što React hidrira) — potpuno bezopasno, nema veze sa našim kodom, standardni "browser extension modifies DOM before hydration" slučaj koji Next.js dokumentacija eksplicitno pominje. Fix: `suppressHydrationWarning` dodat SAMO na `<body>` tag u `layout.tsx` (ne globalno, ne na `<html>`) — sakriva SAMO nepoklapanje atributa na tom tagu, prave hydration greške bilo gde drugde u stablu i dalje će se normalno prijavljivati.

### Title separator — em-dash → pipe
Korisnik promenio SAM (direktno u editoru, van mojih tool poziva) `const title` u `layout.tsx` sa `"Vukašin Riznić — Web Developer"` na `"Vukašin Riznić | Web Developer"`, i isto tako `/projekti` i `/kontakt` `metadata.title` sa " — " na " | " separator. Razlog: čitljivije kad se hoveruje tab u browseru. **Ovo VAŽI SAMO za `<title>`/meta title stringove (browser tab), NE menja postojeće pravilo "bez em-dash u VIDLJIVOM sadržaju stranice"** (heading-ovi, paragrafi i dalje bez crte, to pravilo ostaje na snazi).

## Nerešeno/napomene za oprez

- ⚠️ **NOVO — font-loading dev-mode problem, vidi punu sekciju gore** — ako se sajt na localhost-u ikad opet "skupi"/čudno renderuje bez očigledne veze sa nedavnim izmenama koda, PRVO posumnjaj na ovo (font fallback), ne na CSS/layout kod. Dijagnostika: `document.fonts` + `.next/dev/logs/next-development.log`.
- ⚠️ **`ProjectCard.tsx`/`Services.tsx` slike — pre bilo koje izmene `sizes`/aspect-ratio/dimenzija, pročitaj pune sekcije gore.** Korisnikovo pravilo za Services preview kutiju: širina se sme menjati, visina NE.
- ⚠️ **`document.hidden`/pozadinski tab throttling u ovoj browser-alat sesiji**: kad je Browser pane "u pozadini" (`document.visibilityState === "hidden"`), rAF-driven animacije (Framer Motion `whileInView`, Lenis `scrollTo`) izgledaju KAO DA SU ZAGLAVLJENE ako se provere odmah (`opacity:0` ostaje satima u testu) — ali **NISU pokvarene**, samo throttle-ovane. Ako se sačeka dovoljno (~5s+), animacija SE ZAVRŠI (potvrđeno više puta ovu sesiju: `getAttribute('style')` pre/posle pokazuje tačan prelaz `opacity:0;transform:translateY(28px)` → `opacity:1;transform:none`). **Ne zaključuj da je animacija pokvarena na osnovu trenutne provere** — proveri strukturno (početno stanje se poklapa sa "hidden" varijantom) i/ili sačekaj i proveri ponovo, ili zamoli korisnika da proveri lokalno u svom browseru (najpouzdanije). **VAŽNIJE: ako korisnik pošalje VIDEO snimak sa telefona koji pokazuje realan jank/bug, TO je pouzdaniji dokaz od bilo koje provere u ovoj sesiji — vidi "MOBILE ANIMACIJA JANK" sekciju gore za tehniku analize.**
- ⚠️ **Sve je pushovano na kraju sesije od 2026-08-17** (poslednji commit `039fcd6`) — proveri `git status` na početku svake nove sesije samo kao rutinsku naviku, ne očekuj zaostatak.
- **Vidi "⚠️⚠️ MOBILE ANIMACIJA JANK" gore** — pre bilo kakve izmene reveal/scroll animacija, pročitaj to (CSS transition-all + Framer variants konflikt, i margin/amount trade-off).
