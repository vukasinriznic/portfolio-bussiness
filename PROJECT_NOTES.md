...
- **Afera Digital je korisnikova SOPSTVENA firma** (ne tuđa, potvrđeno pitanjem) — korisnik želi da se njegov portfolio poveže sa pretragom "aferadigital". Firma NEĆE imati sopstveni sajt, SAMO Instagram (još nije napravljen kao od ove sesije). Urađeno: `personJsonLd.worksFor` dodat (`{"@type":"Organization","name":"Afera Digital","sameAs":"https://www.aferadigital.rs"}`), social link labela `lib/links.ts` promenjena sa "Afera"→"**Afera Digital**" (i `Hero.tsx` `mobileSocialPosition` mapa ključ ažuriran da se poklapa, inače bi lookup pukao nečujno). **`href` i dalje pokazuje na `aferadigital.rs` domen koji PO KORISNIKOVIM REČIMA neće ni postojati** — kad korisnik pošalje pravi Instagram URL, promeni `href` na SVA mesta gde se pojavljuje (`lib/links.ts` je jedini izvor, koristi se svuda, tako da je to JEDNO mesto za izmenu).
...
- ⚠️ **NOVO — `document.hidden`/pozadinski tab throttling u ovoj browser-alat sesiji**: kad je Browser pane "u pozadini" (`document.visibilityState === "hidden"`), rAF-driven animacije (Framer Motion `whileInView`, Lenis `scrollTo`) izgledaju KAO DA SU ZAGLAVLJENE ako se provere odmah (`opacity:0` ostaje satima u testu) — ali **NISU pokvarene**, samo throttle-ovane. Ako se sačeka dovoljno (~5s+), animacija SE ZAVRŠI (potvrđeno više puta ovu sesiju: `getAttribute('style')` pre/posle pokazuje tačan prelaz `opacity:0;transform:translateY(28px)` → `opacity:1;transform:none`). **Ne zaključuj da je animacija pokvarena na osnovu trenutne provere** — proveri strukturno (početno stanje se poklapa sa "hidden" varijantom) i/ili sačekaj i proveri ponovo, ili zamoli korisnika da proveri lokalno u svom browseru (najpouzdanije).
- ⚠️ **Puno posla je OSTALO NEPUSHOVANO na kraju ove sesije** (2026-08-17) — commit `a2090fd` (kontakt forma) JE pushovan, ali SVE POSLE toga (animacioni sistem, Lenis nav-scroll, Header fix, sitemap/robots/canonical, Afera Digital rename) je samo lokalno u working tree-u. **PRVA STVAR sledeće sesije: proveri `git status`, pa pitaj korisnika da li da se pushuje pre nastavka rada** — ako se izgubi working directory (nova mašina, novi environment), ovaj rad nestaje.
